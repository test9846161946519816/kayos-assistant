...
def create_user(self):
    ...
    if password == confirm_password :
        if atype == "Administrator" :
            ...
            os.system(useradd)
            os.system(user_password + " | " + "chpasswd -c SHA512")
        GLib.idle_add(_functions.show_in_app_notification, self, "User has been created")
    else:
        GLib.idle_add(_functions.show_in_app_notification, self, "Passwords are not the same")
        _functions.MessageBox(self, "Message", "Passwords are not the same")
