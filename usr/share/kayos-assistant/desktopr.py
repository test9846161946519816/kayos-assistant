...
def check_desktop(desktop):
    ...
    lst = _functions.os.listdir("/usr/share/xsessions/")
    for x in lst:
...
def uninstall_desktop_check(self, desktop):
    dsk = _settings.read_settings("DESKTOP", "default")
...
def check_lock(self, desktop, state):
    if _functions.os.path.isfile("/var/lib/pacman/db.lck"):
        ...
        if result in (Gtk.ResponseType.OK, Gtk.ResponseType.YES):
            _functions.os.unlink("/var/lib/pacman/db.lck")
            # print("YES")
            t1 = _functions.threading.Thread(target=install_desktop,
                                    args=(self,
...
def check_package(self, path, package):
    if _functions.os.path.isfile(path + "/" + package):
        with _functions.subprocess.Popen(["sh", "-c", "yes | pkexec pacman -R " + package], bufsize=1, stdout=_functions.subprocess.PIPE, universal_newlines=True) as p:
...
def install_desktop(self, desktop, state):
    ...
    if not os.path.exists(_functions.home + "/.config-att"):
        os.makedirs(_functions.home + "/.config-att")
    ...
    if os.path.exists(_functions.home + "/.config-att"):
        _functions.permissions(_functions.home + "/.config-att")
    _functions.copy_func(_functions.home + "/.config/", _functions.home + "/.config-att/config-att-" + now.strftime("%Y-%m-%d-%H-%M-%S"), isdir=True)
    _functions.permissions(_functions.home + "/.config-att/config-att-" + now.strftime("%Y-%m-%d-%H-%M-%S"))
    if desktop == "awesome":
        ...
        src.append("/etc/skel/.config/polybar")
        twm = True
    # _functions.subprocess.call(list(np.append(pkexec, command)))

    GLib.idle_add(self.desktopr_prog.set_fraction, 0.2)

    timeout_id = None
    timeout_id = GLib.timeout_add(100, _functions.do_pulse, None, self.desktopr_prog)
    # print(command)
    ...
    if state == "reinst":
        ...
        if self.ch1.get_active():
            ...
            _functions.subprocess.call(["sh", "-c", "yes | pkexec pacman -Scc"], shell=False, stdout=_functions.subprocess.PIPE)
    else:
        com1 = pkexec
    ...
    with _functions.subprocess.Popen(list(np.append(com1, command)), bufsize=1, stdout=_functions.subprocess.PIPE, universal_newlines=True) as p:
        for line in p.stdout:
            GLib.idle_add(self.desktopr_stat.set_text, line.strip())
    ...
    if check_desktop(desktop):
        ...
        if twm is True:
            for x in src:
                if _functions.os.path.isdir(x) or _functions.os.path.isfile(x):
                    print(x)
                    dest = x.replace("/etc/skel", _functions.home)
                    # print(dest)
                    if _functions.os.path.isdir(x):
                        dest = _functions.os.path.split(dest)[0]
                    l1 = np.append(copy, [x])
                    l2 = np.append(l1, [dest])
                    GLib.idle_add(self.desktopr_stat.set_text, "Copying " + x + " to " + dest)

                    with _functions.subprocess.Popen(list(l2), bufsize=1, stdout=_functions.subprocess.PIPE, universal_newlines=True) as p:
                        for line in p.stdout:
                            GLib.idle_add(self.desktopr_stat.set_text, line.strip())
                    # _functions.subprocess.call(list(l2), shell=False, stdout=_functions.subprocess.PIPE)
                    _functions.permissions(dest)

        GLib.idle_add(self.desktopr_stat.set_text, "")
        GLib.idle_add(self.desktop_status.set_text, "This desktop is installed")
        GLib.idle_add(_functions.show_in_app_notification, self, desktop + " has been installed")
    else:
        ...
        GLib.idle_add(_functions.show_in_app_notification, self, desktop + " has not been installed")
