...
def append_repo(self, text):
    with open(_functions.pacman, "a") as myfile:
        myfile.write("\n\n")
        myfile.write(text)

    _functions.show_in_app_notification(self, "Settings Saved Successfully")

    # MessageBox(self, "Success!!", "Settings applied successfully")

def append_mirror(self, text):
    with open(_functions.arcolinux_mirrorlist, "a") as myfile:
        myfile.write("\n\n")
        myfile.write(text)
...
def insert_repo(self, text):
    with open(_functions.pacman, "r") as f:
        lines = f.readlines()
        f.close()
    pos = _functions._get_position(lines, "[custom]")
    num = pos+3

    lines.insert(num, "\n" + text + "\n")

    with open(_functions.pacman, "w") as f:
        f.writelines(lines)
        f.close()


def check_repo(value):
    with open(_functions.pacman, "r") as myfile:
...
def check_mirror(value):
    with open(_functions.arcolinux_mirrorlist, "r") as myfile:
        lines = myfile.readlines()
        myfile.close()
...
def toggle_test_repos(self, state, widget):  # noqa
    if not _functions.os.path.isfile(_functions.pacman + ".bak"):
    ...
    if state is True:
        ...
        try:
            ...
            print(e)
            _functions.MessageBox(self, "ERROR!!",
        ...
        try:
            ...
            _functions.MessageBox(self, "ERROR!!", "An error has occurred setting this setting \'toggle_test_repos Off\'")  # noqa


def toggle_mirrorlist(self, state, widget):  # noqa
    if not _functions.os.path.isfile(_functions.arcolinux_mirrorlist + ".bak"):
        _functions.shutil.copy(_functions.arcolinux_mirrorlist, _functions.arcolinux_mirrorlist + ".bak")
    lines = ""
    if state is True:
        with open(_functions.arcolinux_mirrorlist, 'r') as f:
            lines = f.readlines()
            f.close()
        try:
            for i in range(0, len(lines)):
            ...
            with open(_functions.arcolinux_mirrorlist, 'w') as f:
                # lines = f.readlines()
                f.writelines(lines)
                f.close()
        except Exception as e:
